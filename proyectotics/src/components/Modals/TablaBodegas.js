...
function TablaBodegas() {
  ...
  const [openCreateModal, setOpenCreateModal] = useState(false);  // Estado para controlar el modal de creación de bodega
...
useEffect(() => {
  ...
  const handleButtonClick = (id) => {
    ...
    const stockParts = row.stock.split(' ');  // Separamos el número de la unidad de medida
    setSelectedStock(stockParts[0]);  // Establecemos el valor del número de stock
    setStockUnit(stockParts[1] || '');  // Establecemos la unidad de medida (si existe)
  ...
  const handleSaveStock = () => {
    ...
    setRows(rows.map((row) =>
      row.id === selectedId ? { ...row, stock: `${selectedStock} ${stockUnit}` } : row
    ));
  ...
  const handleSaveNewBodega = () => {
    ...
    // Si no existe, agregamos la nueva bodega
    const newRow = {
      id: rows.length + 1,  // ID único para la nueva fila
      codigo: "Nuevo Codigo", 
    ...
    setRows([...rows, newRow]);  // Añadimos la nueva bodega a las filas
    handleCloseCreateModal();  // Cerramos el modal
  };

  return (
    <div>
      {/* Contenedor con estilo flex para la búsqueda y el botón */}
      <Box sx={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
        ...
        <TextField
          ...
          sx={{ marginTop: 1, width: 250 }}  // Limitar el tamaño del campo de búsqueda
        />
